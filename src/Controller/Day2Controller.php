...
class Day2Controller extends AdventOfCodeBaseController
{
    #[Route(path: '/advent-of-code/day2')]
    public function index(Request $request): Response
    {
        ...
        foreach ($this->getInput() as $input) {
            ...
            [$id, $game] = explode(':', $input);
            $subsets = explode(';', $game);

            foreach ($subsets as $subset) {
                foreach (explode(', ', $subset) as $colorSubset) {
                    [$value, $color] = explode(' ', trim($colorSubset));
                    switch ($color) {
                        case 'red':
                            if ($value > $maxRed) {
                                // Part 1.
            ...
            $list[$id]['green'] = $green;
            $list[$id]['blue'] = $blue;
            $list[$id]['sum'] = $red * $green * $blue;
            $sum += $list[$id]['sum'];
        }
        return new JsonResponse(['total' => $total, 'sum' => $sum, 'list' => $list]);
    }
